...
fullHist <- hist(ancBin$ID)
fullHistBr <- fullHist$breaks
fullDens <- fullHist$counts/sum(fullHist$counts)
plot(ancSmall$Num, type = "l")
ancQuants <- ancVals[,lapply(.SD,function(x){seq(min(x), max(x), length.out = 50)})] ##save quantiles of full set

##function to test KS statistic of LHS distribution
ks_stat_clhs <- function(sample,ancSmall,fullBreaks){
  ...
  smallHist <- hist(sampBin$ID, breaks = fullHistBr)
...
nums <- round(seq(10, 800, length.out = 8))
...
out <- foreach(i = 1:5, .combine = rbind) %do% {
  ...
  randOut <- ks_stat_clhs(sample,ancSmall, ancQuants)
  sample <- createLHS(ancVals,n)
  lhsOut <- ks_stat_clhs(sample,ancSmall, ancQuants)
  data.table(it = i, Random = randOut, LHS = lhsOut)
}

statsAll <- rbind(statsRand,statsCCLHS)
