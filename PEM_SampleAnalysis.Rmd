...
t2 <- temp$data
t2[,NumSamples := 660]
t2[,Label := paste0("660 Samples\nKL = ",round(temp$KL, digits = 3))]
temp <- plotUniDists(s3)
t3 <- temp$data
t3[,NumSamples := 10000]
t3[,Label := paste0(" 10000 Samples\nKL = ",round(temp$KL, digits = 3))]
dat <- rbind(t1,t2,t3)
dat[,Label := as.factor(Label)]
dat[,Label := reorder(Label,NumSamples)]

dat2 <- melt(dat, id.vars = c("Label","ID"), measure.vars = c("Num","SampNum"))
ggplot(dat2, aes(x = ID, y = value, colour = variable, size = variable)) +
  geom_line() +
  facet_wrap(.~Label)+
  theme_bw()+
  scale_color_manual(name = "Source", values = c("red","#0634a4EE"), labels = c("Map Area","cLHS Sample")) +
  scale_size_manual(name = "Source",values = c(1.2,0.8),labels = c("Map Area","cLHS Sample")) +
  labs(x = "Multivariate Bins", y = "Bin ratio of total map area")

```

## Test filling of real data
...
temp <- ancDatVL$extract_points(zonal)[,1:5]
...
temp <- ancDatVL$extract_points(nonzonal)[,1:5]
...
nums <- c(10,200, 400, 500, 600, 700, 800, 900, 1000)
...
statsAll <- statsAll 
ggplot(statsAll, aes(x = Num, y = KL, fill = SType))+
  geom_boxplot()+
   scale_y_continuous(name = "Kullback-Leibler Divergence", limits=c(0, 8),breaks=seq(0,8,1))+
