...
collectStats <- function(X,sampleFun,nums,testFun1 = ks_stat_clhs_v2,testFun2 = fillTest){
  test_res <- foreach(n = nums, .combine = rbind, 
                    .packages = c("Rcpp","philentropy","foreach","clhs"),
                    .export = c("ancQuants","ancSmall","idVals","ancVals")) %dopar% {
...
```{r run simulations}
nums <- round(seq(5, 70, length.out = 10)) ##set sequence of transect number
nums <- rep(nums, each = 15) ##replicates of each number (should be at least 5)
...
pointTime <- collectTime2(allSLDat,pointCCLHS,numNeeded,nSims,plotTime = 2L, minNum = 10)
...
numNeeded <- numTrans[SType == "CCLHS",as.integer(nTrans)]
tspTime <- foreach(n = 1:(nSims-5),.combine = rbind) %do% {
  time <- calcCost_tsp(Xcost,numNeeded,15)
  data.frame(Rep = n, Cost = time)
}
tspTime$SType <- "TSP_Optimised"

timeAll <- rbind(clhsTime,clhsregTime,tspTime)
