...
collectStats <- function(X,sampleFun,nums,testFun1 = ks_stat_clhs_v3, testFun2 = fillTest){
  test_res <- foreach(n = nums, .combine = rbind, 
                    ...
                    .export = c("ancVals")) %dopar% {
                      ...
                      data.frame(Num = n,KL = res1,Fill = res2)
                    }
  return(test_res)
}
```
...
```{r}
s1Dat <- fread("DeceptionFinalAllPoints_cleaned.csv")
...
nums <- round(seq(10, 5000, length.out = 11))
...
numPoints <- temp[,.(nPoints = predict(loess(Num ~ y,data = .SD), data.frame(y = .5))), by = .(SType)]
...
knitr::kable(numPoints, digits = -1, caption = "Number of points for KL divergence = 0.5")
...
nums <- round(seq(5, 100, length.out = 6)) ##set sequence of transect number
nums <- rep(nums, each = 3) ##replicates of each number (should be at least 5)
...
##plot
statsAll2 <- rbind(statsLHS_trans, statsCCLHS_trans, statsCCLHS_30m) #
statsAll2$Num <- as.factor(statsAll2$Num)
statsAll2$SType <- as.factor(statsAll2$SType)
...
ggsave("./Results/KL_Divergence_transects_method.jpeg", device = 'jpeg', width = 6, height=4, units = "in")
...
#temp[,`:=`(y = (y-min(y))/(max(y)-min(y)), Num = as.numeric(as.character(Num)))]
temp[,`:=`(Num = as.numeric(as.character(Num)))]
numTrans <- temp[,.(nTrans = predict(loess(Num ~ y,data = .SD), data.frame(y = 2))), by = .(SType)]
...
The previous chunks investigated how variable space filling changed with different sampling strategies. We now apply the same process, but collect results on the amount of time each plan would take (as determined by the TSP). We use similar sampling plans as in the above chunks: cost-constrained clhs, regular clhs, and TSP (cost-constrained clhs optimised using the TSP). First, we run the simulations as above, with  increasing numbers of transects and multiple replications at each level. We then simulate the cost to fill the variable space to 90% as calculated above, using each of these methods, and present a sample TSP route.
...
trans <- as.integer((seq(5,45,length.out = 5))) ##numbers of transects
...
timeAll <- rbind(SPoint,TSPpoint,clhsTime,clhsregTime,tspTime)
