...
```{r single point}
fullSet <- sampleRegular(ancDatSL, size = 100000,useGDAL = T)
X <- fullSet
...
createLHS <- function(X,n,ancDat){
  ...
  sl_lhs <- clhs(small, size = 25*n, iter = 5000,use.cpp = T,simple = F)
...
collectStats <- function(X,sampleFun,nums,testFun1 = sumKSTest,testFun2 = fillTest,ancDat){
  pkgs <- c("Rcpp","LaplacesDemon", "foreach","sf",
          "raster","LearnGeom","dplyr","reticulate","gdistance","scales","velox")
...
nums <- round(seq(5, 80, length.out = 20))
...
statsTSP_trans <- collectStats(Xcost, createTSP, nums, ancDat = ancDatVL)
statsTSP_trans$SType <- "TSP"
fwrite(statsTSP_trans, "TSPNumSave.csv")
statsCCLHS_30m <- collectStats(Xcost, createCCLHS_30m, nums, ancDat = ancDatSL)
statsCCLHS_30m$SType <- "CCLHSReg"

statsAll <- rbind(statsLHS_trans,statsCCLHS_trans,statsTSP_trans)
statsAll$Num <- as.factor(statsAll$Num)
statsAll$SType <- as.factor(statsAll$SType)
ggplot(statsAll, aes(x = Num, y = KS, fill = SType))+
  geom_boxplot()
...
statsComp <- rbind(statsCCLHS_trans,statsCCLHS_30m)
statsComp$SType <- as.factor(statsComp$SType)
statsComp$Num <- as.factor(statsComp$Num)
ggplot(statsComp, aes(x = Num, y = KS, fill = SType))+
  geom_boxplot()

ggplot(statsComp, aes(x = Num, y = fillQual, fill = SType))+
  geom_boxplot()
```
...
createTSP <- function(X,n,ancDat){
  ...
  small <- small[complete.cases(small),]
  sl_lhs <- c_clhs(small, size = 25*n, iter = 5000)
  temp <- sl_lhs$sampled_data
  return(temp)
}
...
collectTime <- function(X,sampleFun,nsample, reps, dropAllowed){
  pkgs <- c("Rcpp","LaplacesDemon", "foreach","sf",
          "raster","LearnGeom","dplyr","reticulate","gdistance","scales")
  test_res <- foreach(n = 1:reps, .combine = rbind, .noexport = c("CppLHS"), 
                      .packages = pkgs,
                    .export = c("c_clhs","start_sf","tr1", "calcCost")) %dopar% { #
                      reticulate::source_python("mTSP.py")
                      templhs <- sampleFun(X,nsample)
                      dat <- X[templhs$indeces,]
                      if(nrow(dat) > 1){
                        time <- calcCost(dat,templhs$final_obj,dropAllowed)
...
LHSTime <- collectTime(X,timeLHS,nsample = calcNumTrans("LHS",numDat),reps = 14, dropAllowed = F)
LHSTime$Type = "LHS"
CCLHSTime <- collectTime(Xcost,timeCCLHS,nsample = calcNumTrans("CCLHS",numDat),reps = 14, dropAllowed = F)
CCLHSTime$Type = "CCLHS"
TSPTime <- collectTime(Xcost, timeCCLHS, nsample = calcNumTrans("TSP",numDat), reps = 14, dropAllowed = T)
TSPTime$Type = "TSP"

timeAll <- rbind(LHSTime,CCLHSTime,TSPTime)
timeAll$Cost <- timeAll$Cost/480
boxplot(Cost ~ Type, data = timeAll)
```
