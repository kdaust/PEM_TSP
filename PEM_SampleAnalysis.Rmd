...
for(i in layerNamesSl){
  ancDatSL[[i]][ancDatSL[[i]] > quantile(ancVals[[i]],0.9, na.rm = T)] <- NA
}
...
```{r num single points}
statsAll <- as.data.table(statsAll)
temp <- statsAll[,.(y = median(fill)), by = .(SType,Num)]
...
```{r run simulations}
nums <- round(seq(5, 70, length.out = 6)) ##set sequence of transect number
