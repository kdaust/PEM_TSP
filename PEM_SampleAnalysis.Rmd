...
fillTest <- function(full, small){
  strata <- apply(
    ...
    2, 
    function(x) {
      quantile(x, probs = seq(0, 1, 0.005), na.rm = TRUE)
    }
  )
...
# origPC <- prcomp(ancVals,scale. = T, center = T)
# temp <- origPC$x
# hFull <- hist(temp[,1], breaks = 40)
# fullHistBr <- hFull$breaks
# 
# kl_stat_pc <- function(sample){
#   sampPC <- prcomp(sample, scale. = T, center = T)
#   t2 <- sampPC$x
#   t2Vec <- t2[,1]
#   t2Vec <- t2Vec[t2Vec > min(fullHistBr)]
#   hSmall <- hist(t2Vec, breaks = fullHistBr)
#   tempMat <- rbind(hFull$counts,hSmall$counts)
#   KL(tempMat, est.prob = "empirical")
# }
# 
# plot(density(temp[,1]))
# lines(density(t2[,1]))
...
#     fullHist <- hist(ancBin$ID, breaks = as.integer(50))
...
ks_stat_clhs_v3 <- function(sample){
  sampDat <- as.data.table(sample)
  sampDat <- na.omit(sampDat)
  nsamp <- nrow(sampDat)
  ...
  kl2 <- KL(tempMat)
  return(kl2)
  # d1 <- cumsum(fullDat$Num)
  # d2 <- cumsum(fullDat$SampNum)
  # return(max(abs(d1 - d2)))
}

ks_stat_clhs_v4 <- function(sample){
  sampDat <- as.data.table(sample)
  sampDat <- na.omit(sampDat)
  nsamp <- nrow(sampDat)
...
nums <- round(seq(10, 700, length.out = 8))
...
#   randOut <- ks_stat_clhs_v2(sample)
#   sample <- createLHS(ancVals,n)
#   lhsOut <- ks_stat_clhs_v2(sample)
...
ggplot(statsAll, aes(x = Num, y = KL, fill = SType))+
  geom_boxplot()
ggplot(statsAll, aes(x = Num, y = Mean, fill = SType))+
  geom_boxplot()
...
temp <- statsAll[,.(y = median(KL)), by = .(SType,Num)]
temp[,`:=`(y = (y-min(y))/(max(y)-min(y)), Num = as.numeric(as.character(Num)))]
numTrans <- temp[,.(nTrans = predict(loess(Num ~ y,data = .SD), data.frame(y = 0.2))), by = .(SType)]
...
## note this part takes about 10 mins
SPoint <- foreach(n = 1:(nSims*5), .combine = rbind, .packages = pkgs,
                  .export = c("start_sf","trSmall", "calcCost_clhs","allSLDat")) %dopar% { #
                    reticulate::source_python("mTSP.py")
                    templhs <- clhs(allSLDat,size = as.integer(numNeeded/4),cost = ncol(allSLDat),use.cpp = T, iter = 6000,simple = F)
...
SPoint[,TSPNum := rep(c(1:5),each = nSims)]
