...
## list of important stand level covariates (about 11 million r)
#SLcov <- c("tca2.tif","swi_area_mod.tif","tpi.tif")#"twi.tif","valley_depth_2.tif",
...
#   ks2p <- ks2$p.value %>% as.numeric()## returns KS statistic and p value
#   return(ks2p)#kl2, ,ks2p js2, js2, ks2$statistic))
# }
# 
...
#       scale_y_continuous(name = "Bin ratio of total map area", limits=c(0, .12)) + #, ,breaks=seq(0,8,1))
#     scale_color_aaas( name='Source', labels = c('map area', 'cLHS point sample'))+
...
allSLDat <- na.omit(allSLDat) %>% as.data.frame
#allSLDat <- allSLDat[!is.infinite(allSLDat$layer),] ## this line was leading to no observations
...
collectStats <- function(X,sampleFun,nums,testFun1 = KL_stat_clhs, testFun2 = KS_stat_clhs){ ##KS_prob_clhs
  test_res <- foreach(n = nums, .combine = rbind,
                    ...
                    data.frame(Num = n,KL = res1, KS = res2)
                    }
  return(test_res)
}
...
```{r example univariate distributions examples}
# s1 <- createLHS(ancVals,10000)
# s2 <- createLHS(ancVals, 660)
# s3 <- createLHS(ancVals, 100)
# p1 <- plotUniDists(s1)
# p2 <- plotUniDists(s2)
# p3 <- plotUniDists(s3)
# s1stats <- collectStats(ancVals,createLHS, 10000) %>% select(KL, KS) %>% as.numeric() %>% round(3)
# s1.kl.lab <- paste("KL divergence = ", s1stats[1])
# s1.ks.lab <- paste("KS = ", s1stats[2])
# s1.lab <- paste("10000 samples", s1.kl.lab, s1.ks.lab, sep="\n")
# s2stats <- collectStats(ancVals,createLHS, 600) %>% select(KL, KS) %>% as.numeric() %>% round(3)
# s2.kl.lab <- paste("KL divergence = ", s2stats[1])
# s2.ks.lab <- paste("KS = ", s2stats[2])
# s2.lab <- paste("660 samples", s2.kl.lab, s2.ks.lab, sep="\n")
# s3stats <- collectStats(ancVals,createLHS, 100) %>% select(KL, KS) %>% as.numeric() %>% round(3)
# s3.kl.lab <- paste("KL divergence = ", s3stats[1])
# s3.ks.lab <- paste("KS = ", s3stats[2])
# s3.lab <- paste("100 samples", s3.kl.lab, s3.ks.lab, sep="\n")
# #statsLHS$SType <- "LHS"
# library(ggpubr)
# p123 <- ggarrange(p3, p2, p1, ncol = 3, labels = c(s3.lab, s2.lab, s1.lab), widths = c(4, 4, 4),
#                   label.x = 0.14, label.y = .99,
#                   font.label = list(size = 8, color = "black", face = "bold", family = NULL),
#           common.legend = TRUE, legend = "bottom") # Second row with box and dot plots
# 
# plot(p123)
##ggexport(p123, filename = "./Results/sampledistribution.jpg", width = 2000, height = 1000,res = 300)
...
##clhs sample
# colnames(sample) <- names(ancVals)
# comb <- rbind(sample, ancVals)
# newSize <- as.integer(0.9*nrow(sample))
# samp2 <- clhs(comb,size = newSize,can.include = 1:nrow(sample),use.cpp = T, simple = F)
# samp2 <- samp2$sampled_data
# sFull2 <- plotUniDists(samp2)
# ##clhs zonal
# numPoints <- s1Dat[,.(Num = .N), by = .(Unit)]
# zonal <- s1Points[grep("01",s1Points$Unit),]
# temp <- ancDatVL$extract_points(zonal)[,1:7]
# colnames(temp) <- colnames(ancVals)
# temp <- na.omit(temp)
# size <- 334
# samp3 <- clhs(rbind(temp,ancVals),size = size, can.include = 1:nrow(temp),use.cpp = T, simple = F)
# samp3 <- samp3$sampled_data
# nonzonal <- s1Points[!grepl("01",s1Points$Unit),]
# temp <- ancDatVL$extract_points(nonzonal)[,1:7]
# colnames(temp) <- colnames(samp3)
# samp3 <- rbind(temp,samp3)
# sFull3 <- plotUniDists(samp3)

t1 <- sFull1$data
t1[,Label := paste0("All Data\nKL = ",round(sFull1$KL,digits = 3))]
# t2 <- sFull2$data
# t2[,Label := paste0("cLHS to 75%\nKL = ",round(sFull2$KL,digits = 3))]
# t3 <- sFull3$data
# t3[,Label := paste0("cLHS Zonal\nKL = ",round(sFull3$KL,digits = 3))]
# dat <- rbind(t1,t2,t3)
...
#ggsave("./Results/KL_Divergence_Points_method.jpeg", device = 'jpeg', width = 6, height=4, units = "in")
...
clhs_outpath <- file.path("./clhs_sample_plans")
ifelse(!dir.exists(clhs_outpath), dir.create(clhs_outpath, recursive = TRUE), FALSE)

boi <- c ("ESSFmc","ESSFmcw", "SBSmc2") # Deception, 
subzone <- st_read("./Covariates/bec.gpkg")
sample_points <- ""

for (b in boi) {
  #b <- boi[3]
  
  subzone1 <- subzone %>% dplyr::filter(MAP_LABEL %in% b)# %>% 
  subzone_buff <- st_buffer(subzone1, dist = -150)
  boi_mask <- mask(ancDatLL_cost, subzone_buff)# 
  sample_layers_masked <- boi_mask 
...
#         }
#       }
#       if(length(clhs_sampled$acostLL)== slice_size){
...
fwrite(sample_points, paste0("./outputs/",b,  "_theoretical_S1_transectsdesigns2.csv"))
...
createCCLHS <- function(X,n){
  ...
  sl_lhs <- clhs(ancVals, size = nPoints,can.include = idxs,iter = 5000,use.cpp = T,simple = F)
  temp <- sl_lhs$sampled_data
  return(temp)
}
