...
ggplot(dat, aes(x = ID, y = value, colour = variable, size = variable)) +
  geom_line() +
  facet_wrap(.~Label)+
  theme_bw()+
...
samp2 <- clhs(comb,size = newSize,possible.sample = 1:nrow(sample),use.cpp = T, simple = F)
...
temp <- na.omit(temp)
size <- 334
samp3 <- clhs(rbind(temp,ancVals),size = size, possible.sample = 1:nrow(temp),use.cpp = T, simple = F)
