...
test <- ancDatVL$extract(tri)

createCCLHS_LHS <- function(X,n,ancDat){
  xMat <- as.matrix(st_drop_geometry(X))
  temp <- c_clhs(xMat,size = n,i_cost = ncol(xMat),iter = 5000)
  dat <- X[temp$indeces,]
  small <- foreach(j = 1:nrow(dat),.combine = rbind) %do% {
  ...
  sl_lhs <- c_clhs(small, size = 15*n, iter = 5000)
...
statsCCLHS_LHS <- collectStats(Xcost, createCCLHS_LHS, nums, ancDat = ancDatVL)
statsCCLHS_LHS$SType <- "2CCLHS"

statsAll <- rbind(statsLHS_trans,statsCCLHS_trans,statsTSP_trans,statsCCLHS_LHS)
statsAll$Num <- as.factor(statsAll$Num)
statsAll$SType <- as.factor(statsAll$SType)
ggplot(statsAll, aes(x = Num, y = KS, fill = SType))+
  geom_boxplot()
...
cclhsTime <- collectTime(X, timeCCLHS, nsample = 20, reps = 14, dropAllowed = T)
```
