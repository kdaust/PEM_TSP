...
X <- fullSet
X <- X[complete.cases(X),] ##this reduces from 100,000 tp 3700 in one example
...
ks_stat_clhs <- function(sample,fullDat,fullBreaks){
  ...
  cumFull <- cumsum(fullDat$Num)
  cumSamp <- cumsum(fullDat$SampNum)
...
createLHS <- function(X,n,ancDat){
  ...
  sl_lhs <- clhs(ancVals, size = 15*n,possible.sample = idxs,iter = 5000,use.cpp = T,simple = F)
...
collectStats <- function(X,sampleFun, nums,ancDat, testFun1 = ks_stat_clhs,testFun2 = fillTest_Transect){
  ...
  test_res <- foreach(n = nums, .combine = rbind, .packages = pkgs,
                    .export = c("Tri_build","start_sf","tr1","ancQuants","ancSmall")) %dopar% { #
...
```{r run simulations}
nums <- round(seq(5, 50, length.out = 3)) ##set sequence of transect number
...
createTSP <- function(X,n,ancDat){
  ...
  sl_lhs <- clhs(as.data.frame(small), size = 15*n, iter = 5000,use.cpp = T, simple = F)
  temp <- sl_lhs$sampled_data
  return(temp)
}
...
statsTSP_trans <- foreach(n = nums, .combine = rbind, .packages = pkgs,
                    .export = c("Tri_build","start_sf","tr1")) %dopar% { #
                      reticulate::source_python("mTSP.py")
                      smallSet <- createTSP(Xcost,n,ancDatVL)
                      smallSet <- smallSet[complete.cases(smallSet),]
                      if(nrow(smallSet) > 1){
                        res1 <- sumKSTestTransect(ancDatVL,smallSet)
                        res2 <- fillTest_Transect(ancDatVL,smallSet)
                        data.frame(Num = n,KS = res1, fillQual = res2)
                      }else{
                        NULL
                      }
