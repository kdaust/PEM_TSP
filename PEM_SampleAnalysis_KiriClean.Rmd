...
transTime <- collectTime2(Xcost,transSingleNum * 5)
transTime$SType <- "Single Transect"
pairedTime <- collectTime2(Xcost,transPairedNum * 5)
...
SinglePoint <- foreach(n = 1:numSims, .combine = rbind) %do% { #
                    ##reticulate::source_python("mTSP.py")
                    templhs <- clhs(slDatSm,size = as.integer(numNeeded/6),cost = ncol(allSLDat),use.cpp = T, iter = 5000,simple = F)#6000
