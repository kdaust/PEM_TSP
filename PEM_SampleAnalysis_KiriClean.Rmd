...
calcCost_clhs(dat,templhs$final_obj_continuous,45L,3L)
...
transTime <- collectTime2(Xcost,as.integer((transSingleNum * 5)/4))
transTime$SType <- "Single Transect"
pairedTime <- collectTime2(Xcost,as.integer((transPairedNum * 5)/4))
...
allSLDat <- na.omit(allSLDat)
allSLDat <- allSLDat[!is.infinite(acost2),]
numNeeded <- as.integer(numSinglePts)


# pkgs <- c("Rcpp","LaplacesDemon", "foreach","sf",
#         "raster","LearnGeom","dplyr","reticulate","gdistance","scales","clhs")
# .packages = pkgs,
#                   .export = c("start_sf","trSmall", "calcCost_clhs","allSLDat")

## note this part takes about 10 mins (should be nSims*5)
SinglePoint <- foreach(n = 1:numSims, .combine = rbind) %do% { #
                    ##reticulate::source_python("mTSP.py")
                    cat(".")
                    templhs <- clhs(slDatSm,size = as.integer(numNeeded/4),cost = ncol(allSLDat),use.cpp = T, iter = 5000,simple = F)#6000
...
SinglePoint[,SType := "Single Point"]
allStats <- rbind(SinglePoint,transTime,pairedTime)
allStats[,SType := as.factor(SType)]

ggplot(allStats,aes(x = SType, y = Cost)) +
  geom_boxplot()
